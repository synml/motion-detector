    ...
    def update(self, conn, rtsp_url: str):
        ...
        while run:
            ...
            if rec_dat == 1:
                ...
                if win.motionDetector.idleMode == False:

                        win.motionDetector.write_log('이상 감지')
                        win.motionDetector.idleMode = True
                        win.motionDetector.idleInitTime = time.time()


                if win.motionDetector.idleMode == True:
                        win.statusLabel.setText("유휴 상태")
                        win.idleTimeLcd.display(
                            (win.motionDetector.idleInitTime + win.motionDetector.idleTime) - time.time())

                        if win.motionDetector.idleInitTime + win.motionDetector.idleTime <= time.time():
                            win.motionDetector.idleMode = False  # 유휴상태 해제
