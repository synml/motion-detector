...
class Camera(QtCore.QObject):
    ...
    def __init__(self, label, textBrowser):
        ...
        self.camera = cv2.VideoCapture('sample3.avi')
        ...
        _, self.firstFrame = self.camera.read()
    ...
    def loop(self):
        ...
        while self.logic:
            ...
            if self.buffer_frame is None:  # 첫 프레임인 경우에
                self.buffer_frame = self.firstFrame[self.default_y:self.default_y + self.h,
            ...
            # 일반 감지 모드
                ...
                if subtract_frame > self.buffError * self.threshold and self.total_frame >= 3:
                    np.savetxt("np_save/" + str(self.total_frame) + '_error.txt', self.roi_frame, fmt='%1d')
                    # np.save('npz/' + str(self.total_frame) + '_error', self.roi_frame)
                    # print("이상감지")
                    if rasp:
            ...
            np.savetxt("np_save/" + str(self.total_frame) + 'normal.txt', self.roi_frame, fmt='%1d')
            # np.save('npz/'+str(self.total_frame)+'_normal', self.roi_frame)
